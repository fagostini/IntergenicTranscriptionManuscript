...
figureS2c = ggplot(df, aes(x,y)) +
   ggtitle("Independent TU distance to neatest annotated feature") +
   geom_line() +
...
```{r plotSplicing}
counts = readRDS("Count_of_spliced_reads_on_intergenic_and_lncRNA.rds")

tab = rbindlist(lapply(counts, function(x) x[, .(sum(Count)/mean(Total)), by="Subset"]))
tab[Subset=="long_ncRNA", Subset := "Long ncRNA"]
tab[Subset=="newTranscriptionalUnit", Subset := "Independent TU"]
tab[, Subset := factor(Subset, levels=c("Long ncRNA", "Independent TU"))]
...
tab = rbindlist(counts, idcol="Sample")[, list(mean(Count), median(Count), .N), by=c("Sample", "Subset")]
gg1 = ggplot(tab, aes(x=Subset, y=log2(V1))) +
   ggtitle("Distribution of spliced reads") +
   geom_boxplot() +
   scale_x_discrete("") + scale_y_continuous("Average count per dataset (log2)") +
   theme_light()
gg2 = ggplot(tab, aes(x=Subset, y=V2)) +
   ggtitle("") +
   geom_boxplot() +
   scale_x_discrete("") + scale_y_continuous("Median count per dataset") +
   theme_light()
gg3 = ggplot(tab, aes(x=Subset, y=N)) +
   ggtitle("") +
   geom_boxplot() +
   scale_x_discrete("") + scale_y_continuous("Transcript count per dataset") +
   theme_light()
save_and_plot(x=ggarrange(gg1, gg2, gg3, ncol=3), bname="Figures_and_Tables/Splicing_check", width=8, height=8)

tab = rbindlist(counts, idcol="Sample")[Count>1, list(mean(Count), median(Count), .N), by=c("Sample", "Subset")]
gg1 = ggplot(tab, aes(x=Subset, y=log2(V1))) +
   ggtitle("Distribution of spliced reads", subtitle="Transcripts with more than 1 read") +
   geom_boxplot() +
   scale_x_discrete("") + scale_y_continuous("Average count per dataset (log2)") +
   theme_light()
gg2 = ggplot(tab, aes(x=Subset, y=V2)) +
...
gg3 = ggplot(tab, aes(x=Subset, y=N)) +
   ...
   scale_x_discrete("") + scale_y_continuous("Transcript count per dataset") +
   theme_light()
save_and_plot(x=ggarrange(gg1, gg2, gg3, ncol=3), bname="Figures_and_Tables/Splicing_check_gt_1", width=8, height=8)
```

![](./Figures_and_Tables/figureS2d.png)

![](./Figures_and_Tables/Splicing_check.png)

![](./Figures_and_Tables/Splicing_check_gt_1.png)
...
figureS2 = ggarrange(ggarrange(figureS2c, figureS2d, ncol=2, nrow=1, widths=c(3/5, 2/5), align="h", labels=c("A", "B")),
                     ggarrange(figureS2b, figureS2a, ncol=2, nrow=1, widths=c(2/5, 3/5), align="h", labels=c("C", "D")),
                     ncol=1, nrow=2)
save_and_plot(x=figureS2, bname="Figures_and_Tables/Figure_S2", width=14, height=12)
```
...
![](./Figures_and_Tables/Figure_S2.png)
